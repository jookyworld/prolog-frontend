...
import { ArrowLeft, Heart, Users, Clock, Dumbbell } from "lucide-react"
    ...
    <div className="min-h-screen bg-[#101012] text-white pb-32">
      ...
      <div className="px-6 pt-6 space-y-5">
        ...
        <div className="bg-[#17171C] rounded-3xl p-7">
          <div className="mb-4">
            <p className="text-sm text-white/60 mb-2">{routineData.author}님의 루틴</p>
            <h2 className="text-2xl font-bold mb-3 leading-tight">{routineData.title}</h2>
            <p className="text-base text-white/80 leading-relaxed">{routineData.description}</p>
          </div>
          ...
          <div className="flex items-center gap-4 pt-4 border-t border-white/5">
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-[#3182F6]/10 rounded-full">
              <Users className="w-4 h-4 text-[#3182F6]" />
              <span className="text-sm font-bold text-[#3182F6]">{routineData.usageCount}명</span>
        ...
        {/* Exercise List */}
        <div>
          <h3 className="text-lg font-bold mb-4 px-1">운동 종목 ({routineData.exercises.length}개)</h3>
              ...
              <div key={exercise.id} className="bg-[#17171C] rounded-3xl p-6">
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start gap-4">
                    <div className="flex items-center justify-center w-10 h-10 rounded-full bg-[#3182F6]/10 text-[#3182F6] font-bold flex-shrink-0">
                      {index + 1}
                    </div>
                    <div>
                      <h4 className="text-lg font-bold mb-2">{exercise.name}</h4>
                ...
                <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                  <p className="text-xs text-white/50 mb-2">작성자의 최고 기록</p>
                  <div className="flex items-baseline gap-2">
                    {exercise.lastRecord.weight > 0 && (
                      <>
                        <span className="text-2xl font-bold text-[#45FFBC]">{exercise.lastRecord.weight}kg</span>
                        <span className="text-white/60">×</span>
                      </>
                    )}
                    <span className="text-2xl font-bold text-[#45FFBC]">{exercise.lastRecord.reps}회</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-white/70 leading-relaxed">
                이 루틴을 시작하면 작성자의 기록을 참고하며 운동할 수 있어요. 본인의 체력에 맞게 무게를 조절해주세요.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-gradient-to-t from-[#101012] via-[#101012] to-transparent pt-6 pb-8 px-6">
        <Button
